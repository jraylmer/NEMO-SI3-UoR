...
We encountered it already in section \ref{sec:tutorial:subsec:run-directory} to set the directory for the inputs via the \verb|cn_dir| parameter, which we will need to change again here:
...
We can setup following the same general steps as in section~\ref{sec:forcings:subsec:core}, replacing the directory in step~4 with that above, and a few additional changes described below.

\textbf{After step 3, is recommended to set} \verb|nn_fsbc = 1| in namelist group \verb|&namsbc| of \verb|namelist_cfg| (its default value is \verb|2|).
...
Now, continue with the remaining steps \mbox{8--9} from section~\ref{sec:forcings:subsec:core} before submitting the job.
