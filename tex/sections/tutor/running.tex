...
As noted in section \ref{sec:tutor-reference}, it is not recommended to run \NEMOSIcu{} with the PISCES biogeochemical model.
...
\begin{enumerate}\setcounter{enumi}{3}
    ...
    \item\textbf{Set the simulation to run for one year.}
        ...
        For our test runs 1~year is sufficient, so change \verb|nn_itend| to \verb|2920|.

    \item In namelist group \verb|&namdom|, add the parameter \verb|ln_meshmask = .false.|.
...
\endgroup
        ...
        Repeat this in the following namelist groups: \verb|&namsbc_blk|, \verb|&namtra_qsr|, \verb|&namsbc_rnf|, \verb|&nambbc|, and \verb|&namzdf_iwm|.
...
$ ln -s /storage/research/cpom1/share/NEMO/inputs/SETTE/ORCA2_ICE_v5.0.0/eddy_viscosity_3D.nc .
\end{verbatim}
\endgroup
...
Although it is perhaps more work to setup initially, making use of the \verb|cn_dir| parameters is tidier in the long run, particularly when switching to multi-year atmospheric forcing (section~\ref{sec:inputs}), for which a large number of symbolic links would need to be present and maintained in the \verb|EXP00| directory.
...
Customising the outputs is described in section~\ref{sec:outputs}, but for now, it is helpful to replace these two files with the lightweight templates that have been placed in the auxiliary resources, which will configure the outputs to a few essential variables:\footnote{
    If you skip this step and use the original versions of these files (not recommended!), you will need to link the SETTE input \texttt{sali\_ref\_clim\_monthly.nc} in your template run directory as this is needed by one of the default model outputs.
    Alternatively, you can delete the relevant output field from \texttt{file\_def\_nemo-oce.xml} by removing the line with \texttt{field\_ref="sshthster"}.
}
...
These are used to continue a new run starting from where this one finished; section~\ref{sec:inits-restarts} describes how to setup and make use of these restart files.
