...
As an example, we shall extend the three-year run of section~\ref{sec:inputs-core-ciaf} for another three years.

If you look in the run directory of section~\ref{sec:inputs-core-ciaf} (\verb|EXP02| there), as well as the output files previously listed, you will see several files with names beginning \verb|ORCA2_00008760_restart| (assuming the run completed).
These are restart files written at the final time step, which was time step 8760.

Start by configuring a new run directory in the same way as for the original experiment in section~\ref{sec:inputs-core-ciaf}.
